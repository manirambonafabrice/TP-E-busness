...
exports.list_ou = async function (req, res) {
    ...
    try {
      ...
      const query={query:"SELECT so.ID_SERVICE,DESCRIPTION,ABREVIATION,NOM,PRENOM,TELEPHONE from services_ou so left join utilisateur u on so.ID_UT_CHEF=u.ID order by DESCRIPTION"}
    ...
    //   console.log(result)
      await Promise.all(result.map(async val => {
                        ...
                        r.data.push({NAME:val['DESCRIPTION'],ABRE:val['ABREVIATION'],CHEF:val['NOM']?val['NOM']+" "+val['PRENOM']+" "+val['TELEPHONE']:"",ACTION:action+" "+action1})
    ...
    exports.addOu = async function (req, res) {
        ...
        try {         
            ...
            var check=await Model.getOne({table:"services_ou",culomn:{"DESCRIPTION":nom}})
    ...
    exports.addPost = async function (req, res) {
        ...
        try {         
            ...
            var check=await Model.getOne({table:"poste",culomn:{"DESCRIPTION_POSTE":nom}})
                    var n= Object.keys(check).length
    ...
    exports.list_post = async function (req, res) {
        ...
        try {
          // var users = await Model.getOne(table)
          const query={query:"SELECT p.ID_POSTE as ID,DESCRIPTION_POSTE,ABREVIATION_POSTE,DESCRIPTION,NOM,PRENOM,TELEPHONE from poste p left join services_ou so on p.ID_SERVICE=so.ID_SERVICE left join utilisateur u on so.ID_UT_CHEF=u.ID order by DESCRIPTION"}
          var result = await Model.getRequest(query)
        //   var result = await  Model.getList({table:"services_ou"})
          var r={"data":[]}
          await Promise.all(result.map(async val => {
    ...
    // console.log(val)
                            // var action='<div class="btn-group" dropdown><button dropdownToggle type="button" class="btn btn-primary dropdown-toggle">Button dropdown <span class="caret"></span></button><ul *dropdownMenu class="dropdown-menu" role="menu"><li role="menuitem"><a class="dropdown-item" href="#">Action</a></li></ul></div>'
                            var action='<button id="mod-'+val['ID']+'" class="btn btn-primary getDetailsPost" style="margin-right:10px">Modifier</buton>'
                            var action1='<button id="supp-'+val['ID']+'" class="btn btn-danger getDetailsPost" >Supprimer</buton>'
                            
                           
                            r.data.push({NAME:val['DESCRIPTION_POSTE'],ABR:val['ABREVIATION_POSTE'],OU:val['DESCRIPTION'],CHEF:val['NOM']==null?'':val['NOM']+" "+val['PRENOM']+" "+val['TELEPHONE'],ACTION:action+" "+action1})
        ...
        exports.hierarchie_ou = async function (req, res) {
            ...
            try {
                ...
                var ou=await Model.getList({table:"services_ou"})
