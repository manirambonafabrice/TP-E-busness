...
@Component({
  selector: 'app-province-point',
  templateUrl: './province-point.component.html',
  styleUrls: ['./province-point.component.scss']
})
export class ProvincePointComponent implements OnInit {
...
this.commune_filtre=res.communes
  ...
  if(JSON.parse(sessionStorage.getItem("usersession")).data.ID_RAISON>0){
    this.dtOptions_province = {
      ...
      ajax: {
                url:this.connexion.base_url+"routes.transport/list_point_vente_prov/"+this.activatedRoute.snapshot.url[1].path,
                type:"GET", 
                  headers: {
