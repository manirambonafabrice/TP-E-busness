...
//               else  
                  ...
                  if(user[0].STATUT==1||nr>0){
                    ...
                    //  roles.forEach(async val => {
                     ...
                     token= JWT.sign({username: username,pwd:psw}, psw, { expiresIn: 60 * 60 *15 })
                      ...
                      return res.status(200).json({ status: 200,tokenKey:token, data: {ID:user[0].ID,NOM:user[0].NOM,PRENOM:user[0].PRENOM,USERNAME:username,TELEPHONE:user[0].TELEPHONE,EMAIL:user[0].EMAIL,PWD:pwd,PROFIL:r,paths:paths,TYPE_USER:user[0].INTERNE_EXTERNE,FOTO:user[0].FOTO,ID_RAISON:user[0].ID_RAISON,RAISON_LOGO:raison[0].LOGO}, message: "Succesfully Retrieved",response:"ok",type:"probleme" });
        ...
        return res.status(200).json({ status: 200, message: "Veuiller verifier les champs obligatoire",response:"non",type:"probleme"});
    } catch (e) {
...
if(user1[0].ID_RAISON==0){
    var query={query:"SELECT u.ID as ID,u.NOM as NOM,PRENOM,MATRICULE,DATE_EMBAUCHE,USERNAME,SOLDE_CONGE_INITIAL,DATE_SOLDE_CONGE FROM utilisateur u WHERE u.STATUT=1 AND ID<>"+token_key[2]}

}else
    var query={query:"SELECT u.ID as ID,u.NOM as NOM,PRENOM,MATRICULE,DATE_EMBAUCHE,USERNAME,SOLDE_CONGE_INITIAL,DATE_SOLDE_CONGE FROM utilisateur u WHERE u.STATUT=1 AND ID<>"+token_key[2]+" AND ID_RAISON="+user1[0].ID_RAISON}
    ...
    // console.log(req.params.id)
        ...
        // console.log(result)
          ...
          await Promise.all(result.map(async val => {
                            ...
                            // var action='<div class="btn-group" dropdown><button dropdownToggle type="button" class="btn btn-primary dropdown-toggle">Button dropdown <span class="caret"></span></button><ul *dropdownMenu class="dropdown-menu" role="menu"><li role="menuitem"><a class="dropdown-item" href="#">Action</a></li></ul></div>'
                            var action='<button id="'+val['ID_CONGE_DEMAMDE']+'" class="btn btn-primary getModif_conge" style="margin-right:10px">Détail</buton>'
                         
                           if (val['STATUT']==0) {
                              var statut="<span style='color:blue'>CREE</span>" 
                              var date="<span style='color:blue'>"+val['DATE_CREATION']+"</span>" 
                               action+='<button id="mod|'+val['ID_CONGE_DEMAMDE']+'" class="btn btn-success getModif_conge1" style="margin-right:10px">Modifier</buton><button id="supp|'+val['ID_CONGE_DEMAMDE']+'" class="btn btn-danger getModif_conge1" style="margin-right:10px">Supprimer</buton> '
...
if(user1[0].ID_RAISON>0){
    var query={query:"SELECT u.ID as ID,u.NOM as NOM,PRENOM,TELEPHONE,EMAIL,USERNAME,MATRICULE,DATE_EMBAUCHE,u.ID_OU as ID_OU,u.INTERNE_EXTERNE as IEXT,serv.DESCRIPTION as OU,DESCRIPTION_POSTE,RAISON_SOCIAL FROM utilisateur u left join poste p on u.ID_POSTE=p.ID_POSTE left join services_ou serv on u.ID_SERVICE=serv.ID_SERVICE left join raisons_sociales rs on u.ID_RAISON=.rs.ID_RAISON WHERE u.STATUT=1 AND u.ID_RAISON="+user1[0].ID_RAISON+" ORDER BY RAISON_SOCIAL,NOM"}  
}else
    var query={query:"SELECT u.ID as ID,u.NOM as NOM,PRENOM,TELEPHONE,EMAIL,USERNAME,MATRICULE,DATE_EMBAUCHE,u.ID_OU as ID_OU,u.INTERNE_EXTERNE as IEXT,serv.DESCRIPTION as OU,DESCRIPTION_POSTE,RAISON_SOCIAL FROM utilisateur u left join poste p on u.ID_POSTE=p.ID_POSTE left join services_ou serv on u.ID_SERVICE=serv.ID_SERVICE left join raisons_sociales rs on u.ID_RAISON=.rs.ID_RAISON WHERE u.STATUT=1 ORDER BY RAISON_SOCIAL,NOM"}
    ...
    try {
      ...
      await Promise.all(result.map(async val => {
                        ...
                        r.data.push({UD:val['OU'],RAISON:val['RAISON_SOCIAL'],TYPE:type,NAME:val['NOM']+" "+val['PRENOM'],MATRICULE:val['MATRICULE'],DATE_EMBAUCHE:val['DATE_EMBAUCHE'],TEL:val['TELEPHONE'],EMAIL:val['EMAIL'],USERNAME:val['USERNAME'],ROLES:r1,POSTE:val['DESCRIPTION_POSTE'],ACTION:action+" "+action1})
...
if(user1[0].ID_RAISON>0){
        var query={query:"SELECT u.ID as ID,u.NOM as NOM,PRENOM,TELEPHONE,EMAIL,USERNAME,MATRICULE,DATE_EMBAUCHE,u.ID_OU as ID_OU,u.INTERNE_EXTERNE as IEXT,serv.DESCRIPTION as OU,DESCRIPTION_POSTE,RAISON_SOCIAL FROM utilisateur u left join poste p on u.ID_POSTE=p.ID_POSTE left join services_ou serv on u.ID_SERVICE=serv.ID_SERVICE left join raisons_sociales rs on u.ID_RAISON=.rs.ID_RAISON WHERE u.STATUT=0 AND u.ID_RAISON="+user1[0].ID_RAISON+" ORDER BY RAISON_SOCIAL,NOM"}
    }else
    var query={query:"SELECT u.ID as ID,u.NOM as NOM,PRENOM,TELEPHONE,EMAIL,USERNAME,MATRICULE,DATE_EMBAUCHE,u.ID_OU as ID_OU,u.INTERNE_EXTERNE as IEXT,serv.DESCRIPTION as OU,DESCRIPTION_POSTE,RAISON_SOCIAL FROM utilisateur u left join poste p on u.ID_POSTE=p.ID_POSTE left join services_ou serv on u.ID_SERVICE=serv.ID_SERVICE left join raisons_sociales rs on u.ID_RAISON=.rs.ID_RAISON WHERE u.STATUT=0 ORDER BY RAISON_SOCIAL,NOM"}
        ...
        try {
          ...
          await Promise.all(result.map(async val => {
                            ...
                            if(type=='EXTERNE'){type="<span style='color:blue'>ACTIVE DIRECTORY</span>"}else type="FROM DATABASE"
                            r.data.push({UD:val['OU'],RAISON:val['RAISON_SOCIAL'],TYPE:type,NAME:val['NOM']+" "+val['PRENOM'],MATRICULE:val['MATRICULE'],DATE_EMBAUCHE:val['DATE_EMBAUCHE'],TEL:val['TELEPHONE'],EMAIL:val['EMAIL'],USERNAME:val['USERNAME'],ROLES:r1,POSTE:val['DESCRIPTION_POSTE'],ACTION:action+" "+action2})
...
signature=jpg_signature
        ...
        if(req.body.poste==''||req.body.service==null||req.body.service=='null')id_service=NULL
        
        id_post=req.body.poste
        if(req.body.poste==''||req.body.poste==null||req.body.poste=='null')id_post=NULL
        // console.log(req.body)
        if(foto&&signature){
            ...
            "ID_POSTE":id_post,
            "DATE_EMBAUCHE":req.body.date=='null'?NULL:req.body.date,
            "MATRICULE":req.body.matricule,
            "GENRE":req.body.genre,
            "ID_RAISON":req.body.id_raison,
            "FOTO":foto,
            "SIGNATURE":signature
            }
    ...
    try {
        ...
        var psw=(cript(req.body.pwd, req.body.username))
            ...
            var id_user=await Model.insert({table:"utilisateur",culomn:infos})
                ...
                return res.status(200).json({ status: 200, data: {ID:user[0].ID,NOM:user[0].NOM,PRENOM:user[0].PRENOM,USERNAME:req.body.username,TELEPHONE:user[0].TELEPHONE,EMAIL:user[0].EMAIL,PWD:req.body.pwd,PROFIL:r,TYPE_USER:user[0].INTERNE_EXTERNE,FOTO:user[0].FOTO}, message: "Succesfully Retrieved",response:"ok" });
...
signature=jpg_signature
        ...
        if(req.body.date==null)req.body.date=NULL
        id_service=req.body.service
        if(req.body.service==null||req.body.service=='null')id_service=NULL
        id_post=req.body.poste
        // console.log(req.body)
        if(foto&&signature){
...
exports.userInputFill = async function (req, res) {
    ...
    try {
        ...
        const queryraison_={query:"SELECT* from raisons_sociales order by RAISON_SOCIAL"}
        var result_raison = await Model.getRequest(queryraison_)
        ...
        return res.status(200).json({ status: 200,raisons:result_raison, fonctionInfo:object,fonctionInfo1:fonction,serviceInfo:service,posteInfo:poste,response:"ok" , message: "Succesfully Retrieved" });
...
exports.getOne = async function (req, res) {
    ...
    //   console.log(req.params.id)
    const query={query:"SELECT u.ID as ID,u.NOM as NOM,PRENOM,PWD,TELEPHONE,GENRE,DATE_EMBAUCHE,MATRICULE,EMAIL,USERNAME,SOLDE_CONGE_INITIAL,u.ID_SERVICE,u.INTERNE_EXTERNE as IEXT,serv.DESCRIPTION as OU,u.ID_POSTE,DESCRIPTION_POSTE,u.FOTO,SIGNATURE,u.ID_RAISON FROM utilisateur u left join poste p on u.ID_POSTE=p.ID_POSTE left join services_ou serv on u.ID_SERVICE=serv.ID_SERVICE WHERE ID="+req.params.id}
    ...
    //   object = JSON.parse(str);
      ...
      var roles=await Model.getRequest(query1)
         ...
         return res.status(200).json({ status: 200,raisons:result_raison,fonctionInfo:fonction,serviceInfo:service,posteInfo:poste,pwd:psw, user:user,roles:roles,response:"ok" , message: "Succesfully Retrieved" });
...
exports.getUtilisateur = async function (req, res) {
    ...
    try {
        ...
        var users=await Model.getListOrdered({table:"utilisateur",culomn:{"STATUT":1,"ID_RAISON":user1[0].ID_RAISON},order:{COLUMN:"NOM",TYPE:"ASC"}})
                            ...
                            if (j_total>=req.body.nombre) {
                                ...
                                await Promise.all(flux_conge1.map(async val1 => {
                                    ...
                                    await Promise.all(possedants.map(async vl => {
                                        ...
                                        text: " ",
                                        html:"Monsieur/Madame le "+poste[0].DESCRIPTION_POSTE+", Veuillez réagir sur la demande de congé que l'employé(e) "+user[0].NOM+" "+user[0].PRENOM+" ("+user[0].MATRICULE+") vient de soumettre.\n\n <a href='"+req.body.online_url+"notifications/traiter_conge/"+id_conge_demande+"'>Prière de Cliquer ici pour plus d'information</a>"
                                    };
                    ...
                    // console.log(result[0].n)
                            ...
                            await Promise.all(flux_conge1.map(async val1 => {
                                ...
                                await Promise.all(possedants.map(async vl => {
                                    ...
                                    subject: 'DEMENDE DE CONGE',
                                    text: " ",
                                    html:"Monsieur/Madame le "+poste[0].DESCRIPTION_POSTE+", Veuillez réagir sur la demande de congé que l'employé "+user[0].NOM+" "+user[0].PRENOM+" ("+user[0].MATRICULE+")  vient de soumettre.\n\n <a href='"+req.body.online_url+"notifications/traiter_conge/"+id_conge_demande+"'>Prière de Cliquer ici pour plus d'information</a>"
                                };
    ...
    try {
        ...
        j=0
         await Promise.all(result_interim.map(async val => {
            ...
            user1=await Model.getOne({table:"utilisateur",culomn:{"ID":val.USER_ID}})
            // check=await Model.getOne({table:"conge_validation",culomn:{"ID_CONGE_DEMAMDE":id_conge_demande,"ID_POST":user1[0].ID_POSTE}})
            check={query:"SELECT* from conge_validation where ID_CONGE_DEMAMDE="+id_conge_demande+" AND ID_POST="+user1[0].ID_POSTE+" AND STATUT>0"}
            result_check = await Model.getRequest(check)
            var n1= Object.keys(result_check).length
            // console.log(n1)
            if(n1==0){
            ...
            j++
            }else  deja_trater="deja"

            // console.log(deja_trater)
        ...
        return res.status(200).json({ status: 200,response:"ok",deja_trater:deja_trater , message: "Succesfully Retrieved" });
